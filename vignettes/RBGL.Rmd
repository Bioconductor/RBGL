...
vignette: >
  ...
  %\VignetteIndexEntry{*RBGL*: R interface to boost graph library}
...
in Figure [4](#fig4){reference-type="ref" reference="fdpic"}.
...
<a id="fig4"></a>
```{r fdpic, fig.width = 4, fig.height = 4, fig.cap = "File dependency digraph example from Boost", echo = FALSE}
plot(FileDep)
```
...
```{r figdfs, fig.cap = "a) The graph for depth-first-search example b) The graph for depth-first-search example, showing search orders.", echo = FALSE}
...
```{r figbfs, fig.cap = "a) The graph for breadth-first-search example b) The graph for breadth-first-search example, showing search orders.", echo = FALSE}
layout(layout_matrix_2)
...
```{r figospf, fig.width = 6, fig.cap = "Network example from BGL.", echo = FALSE}
plot(ospf)
```
...
```{r figjoh, fig.small = TRUE, fig.cap = "Example: Johnson-all-pairs-shortest-paths example", echo = FALSE} 
plot(zz)
```
...
```{r figkm, fig.cap = "Kruskal MST examples.", echo = FALSE}
layout(layout_matrix_2)
plot(km, main="g) Kruskal MST Example")
...
```{r figcoex2i, fig.small = TRUE, fig.cap = "Example conn2i", echo = FALSE}
plot(coex2i)
```
