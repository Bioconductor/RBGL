...
%%LL: seems there are edge weights from the graph... need to look at the codes
TO-BE-VERIFIED: Since no edge weights are available, {\em length} and {\em pweights} are all {\em NA}.
