...
<<figkmst, fig=TRUE, echo=FALSE, results=hide, include=FALSE  >>=
  plot(km, main="d) Kruskal MST Example")
@
...
e) The graph for the Kruskal MST example. }
\end{figure}
...
a) use neato from Rgraphviz.
b) use crudeGraphPlot with default.
c) use crudeGraphPlot with spring layout. }
\end{figure}
