...
dfsNattrs$label[dfs.res$discovered] <- paste(dfs.res$discovered, 1:numNodes(df))
z <- plot(df, "neato", nodeAttrs=dfsNattrs) 
@
\caption{DFS example: numbers indicates the discovered order of the node}
\end{figure}
...
bfsNattrs$label[bfs.res] <- paste(bfs.res, 1:numNodes(bf))
z <- plot(bf, "neato", nodeAttrs=bfsNattrs) 
@
\caption{BFS example: numbers indicates the discovered order of the node}
\end{figure}

\subsection{Shortest paths}

TO-BE-FINALIZED:
Possibilities between two vertices:
...
(1) find shortest paths between a single vertex and all other vertices, 
available algorithms are: {\em Dijkstra's}, {\em Bellman Ford's} and {\em DAG},
and 
(2) find shortest paths between all pairs of vertices, available algorithms are: {\em Johnson's} and {\em Floyd Warshall's}.
...
v2 <- "F"
v3 <- "A1"
same.component(jcoex, v1, v2)
same.component(jcoex, v1, v3)
@
...
weight into account when removing edges and outputs the vertices on the 
...
TODO: in "graphviz", "neato" uses Kamada-Kawai and "fdp" uses 
...
are set randomly by calling {\em randomGraphLayout}.  Notice that the choice 
of "width" and "height" values has dramatic impact on the performance.
...
edges.  There are more vertices in example {\em conn2} than {\em dijkstra}, so
...
%%LL: yes, I checked them and it seems containing all the edges.  Self-loops
%% are presented in BGL's example, as well.
