...
The \Rfunction{prim.minST} function finds the MST by Prim's algorithm.
...
#km <- fromGXL(file(system.file("GXL/kmstEx.gxl",package="graph")))
km1 <- km
km1@nodes <- c(km@nodes,"F","G","H")
km1@edgeL$F <- list(edges=numeric(0))
km1@edgeL$G <- list(edges=8, weights=1)
km1@edgeL$H <- list(edges=7, weights=1)
km1@edgemode <- "undirected"
connectedComp(ugraph(km1))
@
\begin{figure}
<<fig=TRUE, echo=false>>=
km1@edgemode <- "directed"
z <- plot(km1)
@
...
#km <- fromGXL(file(system.file("GXL/kmstEx.gxl",package="graph")))
km2 <- km
km2@nodes <- c(km@nodes,"F","G","H")
km2@edgeL$F <- list(edges=numeric(0))
km2@edgeL$G <- list(edges=8, weights=1)
km2@edgeL$H <- list(edges=7, weights=1)
strongComp(km2)
@
...
jcoex@edgeL$A <- list(edges=c(2, 3, 4, 18))
incremental.components(jcoex)
same.component(jcoex, "A", "A1")
@
...
<<fig=TRUE, echo=false>>=
#z <- plot(jcoex)
# REMOVEME: "graph" needs to address the following:
# The graph is undirected and the following edges are not reciprocated:
# B4|A
#
#Error in .local(x, y, ...) : The graph to be plotted is not a valid graph structure
#
@
...
The functions, \Rfunction{edmunds.karp.max.flow} and 
...
Call to \Rfunction{edmunds.karp.max.flow} finds the maximum flow of 2 from 
...
%%LL: in "graphviz", "neato" uses Kamada-Kawai and "fdp" uses 
%%Fruchterman-Reingold.  Probably we should just skip this part completely???

The \Rfunction{randomGraphLayout} function puts the vertices randomly 
on a plane;

The \Rfunction{circle.layout} function puts the vertices as vertices of 
a regular polygon;

The \Rfunction{kamada.kawai.spring.layout} function is for connected, 
...
are set randomly by calling \Rfunction{randomGraphLayout}.  
Notice that the choice of "width" and "height" values has dramatic 
...
You can use Rgraphviz package to do the actual layouts: 
...
If you want to draw the graphs based on the calculated coordinates, you need additional function.  The following is an example.
...
Now crudeGraphPlot(coex) gives the default circular layout on your graphics device.

crudeGraphPlot(coex, alg=kamada.kawai.spring.layout, col="green") shows you the spring layout with green edges.
...
separates("B", "A", "E", km)
# REMOVEME: "graph" needs to address this
#separates("B", "A", "C", km)
@

\subsection{kCores}
Function {\em kCores} finds all the k-cores in an undirected graph.  It returns the core numbers for all the nodes in the graph.
...
<<kCliquesdemo>>=
kclex <- fromGXL(file(system.file("XML/snacliqueex.gxl",package="RBGL")))
kCliques(kclex)
@
